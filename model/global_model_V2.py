...
class GlobalModel(torch.nn.Module):
    def __init__(self,dataset,out_dim,only_base_gnn,only_mhc,use_together,Base_GNN=None,base_dropout=0.5,ks_layer='gcn',mhc_dropout=0.5,base_layer=2,mhc_layer=1,mhc_num_hops=3,separate_conv = True,jk = 'concat',feature_fusion='weighted',combine='geometric',use_ppgn=False,nips=4,pooling_method='sum', *args, **kwargs):
        ...
        if only_base_gnn:
            ...
            if not use_ppgn:
                ...
                self.models.append(PPGN(ninp=nips,hidden2=out_dim))
        if only_mhc:
            print ('use mhc............')
            if ks_layer=='gcn':
                # data,dim=128,dropout=0.5,k=4,separate_hop_conv=True,combine='geometric',layers = 2,feature_fusion='average',JK='concat'
            ...
            if ks_layer=='gin':
                ...
                self.models.append(GINE_Model(dataset,out_dim,mhc_layer,mhc_dropout,mhc_num_hops,feature_fusion,JK=jk,pooling_method=pooling_method))
        if use_together:
            print ('use both.......................')
            if not use_ppgn:
                self.gnn = Base_GNN(dataset, base_layer, out_dim, base_dropout,pooling_method=pooling_method)
            else:
                self.gnn = PPGN(ninp=nips,hidden2=out_dim)
            if ks_layer == 'gcn':
                self.ks_model = GCNE_Model(dataset,out_dim,mhc_dropout,mhc_num_hops,separate_conv,combine,mhc_layer,feature_fusion,JK=jk)
            if ks_layer=='gin':
                # data, hidden=64, layers=3, dropout=0.5,k=4,feature_fusion='average',JK='last'
                self.ks_model = GINE_Model(dataset,out_dim,mhc_layer,mhc_dropout,mhc_num_hops,feature_fusion,JK=jk,pooling_method=pooling_method)
            self.models.append(self.gnn)
            self.models.append(self.ks_model)
