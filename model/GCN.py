...
class GCNE_Model(nn.Module):
    def __init__(self,data,dim=128,dropout=0.5,k=4,separate_hop_conv=True,combine='geometric',layers = 2,feature_fusion='average',JK='concat'):
        super().__init__()
        self.k = k
        in_dim = data.num_features+data[0].rw_feature.shape[1]
        self.node_emb = nn.Linear(in_dim,dim)
        self.edge_emb = nn.Linear(data.num_edge_features,dim,bias = False)
        self.JK = JK
        self.convs = nn.ModuleList([GCNE_Block(dim,dropout, k, separate_hop_conv,combine) for _ in range(layers)])
...
class GCNE_Block(nn.Module):
    def __init__(self,dim=128,dropout=0.5,k=4,separate_hop_conv=True,combine='geometric'):
        super().__init__()
        self.k = k
        self.hop_ind = [f'hop{i+1}' for i in range(k)]
        self.separate_conv = separate_hop_conv
        if separate_hop_conv:
            self.convs = nn.ModuleList([GCNEConv(dim,dim) for _ in range(k)])
        else:
            self.convs = nn.ModuleList([GCNEConv(dim,dim)])
    ...
    def forward(self, data):
        ...
        if 'edge_attr' in data:
            ...
            val,edge_attr = self.convs[0](data.x,hop_matrices[0],edge_attr)
            out.append(val)
            data.edge_attr = edge_attr
        else:
            val = self.convs[0](data.x,hop_matrices[0],None)
            out.append(val)
        if self.separate_conv:
            for i in range(1,self.k):
                val = self.convs[i](data.x,hop_matrices[i],None)
                out.append(val)
        else:
            for i in range(1,self.k):
                val = self.convs[0](data.x,hop_matrices[i],None)
                out.append(val)
