...
if __name__ =='__main__':
    ...
    if fold_index==-1:
        ...
        if is_tudataset:
            ...
            now = get_date_str()
            file_params = f'date_{now}_{args.dataset_name}_layer_{mhc_layer_num}_hop_{mhc_num_hops}_wd_{weight_decay}_lr_{lr}.txt'
            with open(f'result/{file_params}','w') as f:
                f.writelines(info)
        ...
        metrics = run_one_fold(**params)
        saved_path = f'result/separate_run/{args.dataset_name}/date_{now}/params_{mhc_layer_num}_{mhc_num_hops}/fold_{fold_index}/'
